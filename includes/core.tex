...
Keď sa pozrieme na dôležitosť týchto systémov, v určitých odvetviach zohrávajú dôležitú rolu. Väčšina veľkých internetových služieb ako Amazon, YouTube, Netflix, Google, Tripadvisor či \acrshort{imdb}, majú vyvinuté svoje vlastné odporúčacie systémy, ktoré dlhodobo pomáhajú zlepšovať ich výsledky, ale aj UX. Tak isto aj mnoho mediálnych firiem už dnes vyvíja odporúčacie systémy, ktoré potom nasadzujú a používajú ako časť služby, ktorú poskytujú svojim odoberateľom. Netflix v roku 2009 ocenil tím programátorov, ktorý ako prvý úspešne zefektívnil ich odporúčací systém o takmer 10\%, cenou 1 milión dolárov. To len podčiarkuje ich významnosť a fakt, že svetové firmy do tejto technológie investujú nemalé finančné prostriedky.  \\
...
Priečinok \textit{src} obsahuje podpriečinok \textit{API} v ktorom sú súbory týkajúce sa Firebase autentifikácie a práce s databázou Firestore. V podpriečinku \textit{assets} sa nachádzajú použité fonty a pozadia. Priečinok \textit{components} združuje všetky nami vytvorené komponenty. V neposlednom rade máme podpriečinok \textit{screens}, v ktorom sa nachádzajú všetky obrazovky a ich štýlovania.

Priečinok \textit{navigation} obsahuje rôzne druhy navigácií, ktorým sa budeme venovať v \hyperref[sec:screens]{ďalšej časti} implementácie.
...
Vložili sme teda doň nami vytvorené obrazovky SignIn (na prihlasovanie), SignUp (na registráciu), Home (ako domácu obrazovku) a MovieDetail (ako detail filmu). Na obrazovky, ktoré sú takto priamo vložené do stack navigatoru, je možné sa dostať vrámci aplikácie z akejkoľvek inej obrazovky a navigácie. Ostatné obrazovky, ktoré majú špecifické použitie, sú použité vrámci ďalších navigácií, pričom tie budú vo vnútri tohto hlavného stack navigatora. 
\subsubsection{Drawer navigation}
Drawer navigation je typ navigácie, ktorý pôsobí ako menu, ktoré sa dá otvoriť potiahnutím do strany na obrazovke. V samostatnom súbore DrawerNavigator.js sme ho vytvorili pomocou funkcie \textit{createDrawerNavigator()}. Doň sme vložili obrazovku Home, MyProfile a Requests.
\subsubsection{Tab navigation}
Tab navigation slúži na prepínanie medzi viacerými obrazovkami vo vytvorenej spodnej lište. Vytvorili sme ho taktiež v samostatnom súbore TabNavigator.js pomocou funkcie \textit{createBottomTabNavigator()}.
\subsubsection{Dokončenie navigácie}
Na úspešné spustenie navigácie, bolo treba pridať hlavný stack navigator do komponentu NavigationContainer v hlavnom súbore App.js (\hyperref[appjs-navDone]{obrázok \ref{appjs-navDone}}). 
\begin{lstlisting}[caption={App.js po implementácií navigácie}, label={appjs-navDone}]
...
//TODO
Ďalšou dôležitou časťou aplikácie je nepochybne autentifikácia používateľa. Keďže sme si vybrali riešenie od Firebase, pri ktorom všetka implementácia okolo overovania a prihlasovania prebieha n
