...
Na získavanie feedbacku od používateľa sa používajú dve spôsoby: 
\begin{itemize}[leftmargin=*]
...
To nám vnuklo myšlienku, že by sa tento proces výberu dal zjednodušiť moderným riešením pomocou mobilnej aplikácie, založenej na odporúčacích systémoch. V rámci implementácie, sme sa rozhodli, že by bolo vhodné, aby fungovala pre oba populárne operačné systémy (Android aj iOS), vďaka čomu používatelia nebudú obmedzovaní a viazaní na jeden operačný systém. Zvolením frameworku React Native sme teda zabezpečili vytvorenie tzv. cross platformovej aplikácie (\hyperref[sec:typy aplikacii]{kapitola 1.2.}). 
Používateľ by v nej vedel hodnotiť filmy swipeovaním a aplikácia by sa postupne učila jeho preferenciám. Pri spojení dvoch používateľov by aplikácia na základe ich interakcií s filmami vygenerovala zoznam filmov, z ktorého si obaja budú môcť vybrať tie, ktoré ich najviac zaujmú. Aplikácia im následne bude vedieť zobraziť zhody v ich hodnoteniach.
...
Na iOS v čase zadania práce podobná aplikácia nebola k dispozicií. To sa neskôr zmenilo, keď v marci tohto roku vyšla aplikácia Movie Match. Funkcionalitou a vzhľadom je na tom omnoho lepšie ako spomínaná Android aplikácia. Ako nevýhodu však považujeme, že ak sa používateľ chce spojiť s iným používateľom, musí to riešiť len zadaním jeho mailu, na ktorý mu aplikácia pošle pozvánku vo forme mailu. Toto riešenie je nepraktické, najmä ak je už používateľ v aplikácií zaregistrovaný. Intuitívnejším riešením by bolo mať takéto pozvánky zobrazené priamo v aplikácií.


\subsection{Návrh riešenia}
Cieľom tejto bakalárskej práce je navrhnúť a implementovať mobilnú aplikáciu, 
ktorá bude slúžiť ako pomocník pri spoločnom výbere filmu dvoch používateľov, ktorí sa v nej navzájom prepoja. Ná základe ich vybudovaného profilu preferencií jednotlivých atribútov filmov, sa im vygenerujú odporúčané filmy, ktoré budú mocť obaja hodnotiť a následne si pozrieť zhody. Používateľ bude vedieť hodnotiť filmy aj samostatne bez nutnosti byť prepojený s iným používateľom. Tým si môže postupne budovať svoj vlastný profil preferencií. 

Všetka logika aplikácie prebieha na klienstkej strane aplikácie, keďže komplexnosť aplikácie si nevyžadovala rozdelenie na frontend a backend. Na vytvorenie aplikácie sme použili framework React Native, pričom na ukladanie dát nám slúži cloudová NoSQL databáza Firestore \hyperref[sec:firestore]{(kapitola 1.7.2)}. Ako zdroj filmových dát sme zvolili API s názvom TMDb.

V ďalšej časti práce je návrh detailnejšie zvizualizovaný pomocou vybraných UML diagramov. 
...
\begin{figure}[!htbp]
  \centering  
  \def\stackalignment{c}
	\stackunder{\includegraphics[width=15cm]{img/usecase1.png}}%
           {\scriptsize}
...
\subsubsection{Konceptuálny model}
